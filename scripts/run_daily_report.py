...
def main() -> None:
    ...
    stem = ROOT / f"ipo_daily_analysis_{report_date.isoformat()}"
    ...
    print(f"report_id={report['id']} version={report['version']} items={report['item_count']}")
...
def funding_conflict(data: list[dict]) -> str:
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>港股 IPO 分析｜{report_date.isoformat()}</title><style>
:root{{--navy:#0b1739;--red:#e10d15;--bg:#f4f6f9;--line:#dce3ec;--muted:#586380;--green:#0b9f61;--amber:#db7a00}}*{{box-sizing:border-box}}html,body{{margin:0;min-height:100%;font-family:"PingFang SC","Microsoft YaHei",Inter,system-ui,sans-serif;color:var(--navy);background:var(--bg)}}button,select{{font:inherit}}button{{cursor:pointer}}.topbar{{height:64px;background:#fff;border-bottom:1px solid var(--line);display:flex;align-items:center;padding:0 28px;gap:52px;position:sticky;top:0;z-index:30}}.brand{{font-size:21px;font-weight:800;white-space:nowrap}}.nav{{height:100%;display:flex;gap:14px}}.nav button{{border:0;background:none;padding:0 20px;color:#3a455e;font-weight:650;position:relative}}.nav button.active{{color:var(--red)}}.nav button.active:after{{content:"";position:absolute;height:2px;background:var(--red);left:14px;right:14px;bottom:0}}.source{{margin-left:auto;color:var(--muted);font-size:12px}}.source i{{display:inline-block;width:7px;height:7px;background:#17a66a;border-radius:50%;margin-right:7px}}.shell{{display:grid;grid-template-columns:minmax(720px,1fr) 430px;min-height:calc(100vh - 64px)}}.main{{padding:30px 28px 60px;min-width:0}}.heading{{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px}}.heading h1{{font-family:Georgia,"Songti SC",serif;font-size:32px;margin:0 0 6px}}.heading p{{margin:0;color:var(--muted);font-size:12px}}.primary{{border:0;border-radius:7px;background:var(--red);color:#fff;font-weight:700;padding:13px 19px;box-shadow:0 6px 16px #e10d1530;text-decoration:none;display:inline-flex;gap:8px;align-items:center}}.summary{{background:#fff;border:1px solid var(--line);border-radius:10px;display:grid;grid-template-columns:repeat(5,1fr);padding:16px 5px;margin-bottom:24px}}.stat{{text-align:center;padding:0 8px;border-right:1px solid var(--line)}}.stat:last-child{{border:0}}.stat span{{display:block;color:var(--muted);font-size:12px;margin-bottom:6px}}.stat strong{{font-size:23px}}.green{{color:var(--green)}}.amber{{color:var(--amber)}}.red{{color:var(--red)}}.fund-plan{{background:#fff;border:1px solid var(--line);border-radius:10px;padding:17px 20px;margin:-8px 0 22px;display:flex;align-items:center;gap:22px}}.fund-plan strong{{font-size:18px}}.fund-plan p{{margin:3px 0 0;color:var(--muted);font-size:11px}}.fund-bars{{display:flex;flex:1;height:10px;border-radius:8px;overflow:hidden;background:#edf0f4}}.fund-bars i:nth-child(1){{background:#0b1739}}.fund-bars i:nth-child(2){{background:#25375c}}.fund-bars i:nth-child(3){{background:#0b9f61}}.fund-bars i:nth-child(4){{background:#e5a02f}}.fund-bars i:nth-child(5){{background:#e10d15}}.filters{{display:flex;align-items:flex-end;gap:16px;margin-bottom:14px}}.filters label{{font-size:12px;color:#4e5a72}}.filters select{{display:block;margin-top:6px;min-width:150px;padding:9px 34px 9px 11px;border:1px solid var(--line);border-radius:7px;background:#fff;color:var(--navy)}}.reset{{height:38px;border:1px solid var(--line);background:#fff;border-radius:7px;padding:0 16px;color:#34405a;font-weight:600}}.table-wrap{{background:#fff;border:1px solid var(--line);border-radius:10px;overflow:auto}}.ipo-table{{width:100%;border-collapse:collapse;min-width:960px;font-size:14px}}.ipo-table th{{text-align:left;padding:15px 12px;border-bottom:1px solid var(--line);font-size:13px;font-weight:700}}.ipo-table td{{padding:14px 12px;border-bottom:1px solid #edf0f5;color:#37425b}}.ipo-table tbody tr{{cursor:pointer}}.ipo-table tbody tr:hover{{background:#f9fafc}}.ipo-table tbody tr.selected{{box-shadow:inset 3px 0 var(--red)}}.company{{display:flex;gap:9px;align-items:flex-start;min-width:180px}}.radio{{width:14px;height:14px;border:1px solid #a4aec0;border-radius:50%;margin-top:3px;flex:none}}.selected .radio{{border:4px solid var(--red)}}.company strong{{display:block;color:var(--navy);margin-bottom:3px;font-weight:700}}.company small{{color:#6b7588}}.price small{{display:block;color:#8a94a6;font-size:9px}}.tier{{display:inline-block;padding:6px 10px;border-radius:5px;font-weight:700;white-space:nowrap;font-size:13px}}.tier.priority{{background:#e5f7ef;color:#0b9f61}}.tier.cautious{{background:#fff3df;color:#d47800}}.tier.avoid{{background:#feecee;color:#d20e15}}.foot{{font-size:12px;color:var(--muted);margin:10px 3px}}.panel{{background:#fff;border-left:1px solid var(--line);height:calc(100vh - 64px);position:sticky;top:64px;overflow:auto;padding:24px}}.panel-head{{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:1px solid var(--line);padding-bottom:17px}}.panel-head h2{{font-size:20px;margin:0 6px 0 0;display:inline}}.panel-head .code{{font-size:12px;color:var(--muted)}}.close{{border:0;background:none;font-size:27px;line-height:1}}.panel-meta{{font-size:12px;color:var(--muted);margin:12px 0 24px}}.panel section{{border-bottom:1px solid var(--line);padding-bottom:20px;margin-bottom:20px}}.panel h3{{font-size:14px;margin:0 0 14px}}.dimension{{display:grid;grid-template-columns:92px 1fr 36px;gap:8px;align-items:center;margin:13px 0;font-size:12px}}.dimension label{{color:#39455e}}.meter{{height:6px;border-radius:6px;background:#e8ecf2;overflow:hidden}}.meter i{{display:block;height:100%;background:var(--navy);border-radius:6px}}.equation{{display:grid;grid-template-columns:1fr auto 1fr auto 1fr;align-items:center;text-align:center}}.equation span{{font-size:10px;color:var(--muted);display:block}}.equation strong{{font-size:22px}}.equation b{{color:var(--red);font-size:22px}}.copy{{font-size:12px;color:#4c5870;line-height:1.75}}.risks{{margin:0;padding-left:18px;color:#4c5870;font-size:12px;line-height:1.75}}.mobile-open{{display:none}}@media(max-width:1100px){{.shell{{display:block}}.main{{padding-right:28px}}.panel{{position:fixed;right:0;top:64px;width:min(430px,100vw);z-index:25;box-shadow:-12px 0 30px #0b173920}}.panel.closed{{display:none}}.mobile-open{{display:block;position:fixed;right:18px;bottom:18px;border:0;border-radius:50%;width:48px;height:48px;background:var(--red);color:#fff;z-index:22;box-shadow:0 6px 18px #e10d1540}}}}@media(max-width:700px){{.topbar{{height:58px;padding:0 15px;gap:10px}}.brand{{font-size:18px}}.nav{{display:none}}.source{{font-size:0}}.shell{{min-height:calc(100vh - 58px)}}.main{{padding:20px 12px 50px}}.heading{{align-items:flex-start}}.heading h1{{font-size:26px}}.heading p{{max-width:210px}}.primary{{padding:10px 12px;font-size:12px}}.summary{{grid-template-columns:repeat(2,1fr)}}.stat{{padding:9px;border-bottom:1px solid var(--line)}}.filters{{display:grid;grid-template-columns:1fr 1fr}}.filters select{{min-width:0;width:100%}}.reset{{grid-column:1/-1}}.panel{{top:58px;height:calc(100vh - 58px)}}}}
</style></head><body><header class="topbar"><div class="brand">港股IPO分析</div><nav class="nav"><button class="active">今日分析</button><button data-nav="ipos">IPO项目</button><button data-nav="history">历史日报</button><button data-nav="rules">评分规则</button></nav><div class="source"><i></i>数据源：{html.escape(source_path.name)} · 截至 {generated_at}</div></header><div class="shell"><main class="main"><div class="heading"><div><h1>今日 IPO 分析</h1><p>{len(data)}只仍可申购新股 · YAML 10分制 · 已剔除截止/上市项目</p></div><a class="primary" href="ipo_daily_analysis_{report_date.isoformat()}.md" download>▣&nbsp; 下载 Markdown 日报</a></div><div class="summary"><div class="stat"><span>今日项目数</span><strong id="count">{len(data)}</strong></div><div class="stat"><span>申购</span><strong class="green">{counts['申购']}</strong></div><div class="stat"><span>观望</span><strong class="amber">{counts['观望']}</strong></div><div class="stat"><span>回避</span><strong class="red">{counts['回避']}</strong></div><div class="stat"><span>平均规则分</span><strong>{average:.1f}</strong></div></div><section class="fund-plan"><div><strong>HK$50,000 现金方案</strong><p>{html.escape(fund_text)}</p></div><div class="fund-bars" aria-label="资金分配">{fund_bars}</div></section><div class="filters"><label>行业<select id="industry"><option value="">全部</option></select></label><label>推荐<select id="tier"><option value="">全部</option><option>申购</option><option>观望</option><option>回避</option></select></label><button class="reset" id="reset">↻&nbsp; 重置</button></div><div class="table-wrap"><table class="ipo-table"><thead><tr><th>公司</th><th>代码</th><th>行业</th><th>招股价</th><th>截止日期</th><th>认购倍数</th><th>综合分</th><th>建议</th></tr></thead><tbody id="rows"></tbody></table></div><div class="foot" id="foot"></div></main><aside class="panel" id="panel"><div class="panel-head"><div><h2 id="pname"></h2><span class="code" id="pcode"></span> <span id="ptier"></span></div><button class="close" id="close" aria-label="关闭">×</button></div><div class="panel-meta" id="pmeta"></div><section><h3>评分维度 <small style="font-weight:400;color:#7b869a">（原始分 / 权重）</small></h3><div id="dims"></div></section><section><div class="equation"><div><span>基础分</span><strong id="base"></strong></div><b>+</b><div><span>调整</span><strong>0</strong></div><b>=</b><div><span>最终分</span><b id="final"></b></div></div></section><section><h3>公司质地</h3><div class="copy" id="quality"></div></section><section><h3>风险提示</h3><ul class="risks" id="risks"></ul></section><section style="border:0"><h3>研究结论</h3><div class="copy" id="conclusion"></div></section></aside></div><button class="mobile-open" id="open">≡</button><script>const DATA={data_json};const weights=[1,1,1,3,3,1],labels=['基石投资者','基石质量','绿鞋机制','公开申购倍数','估值吸引力','保荐人'];let selected=DATA[0];const $=id=>document.getElementById(id);function tierClass(t){{return t==='申购'?'priority':t==='观望'?'cautious':'avoid'}}function render(){{const ind=$('industry').value,tier=$('tier').value;const list=DATA.filter(x=>(!ind||x.industry===ind)&&(!tier||x.tier===tier));$('rows').innerHTML=list.map(x=>`<tr data-code="${{x.code}}" class="${{selected&&selected.code===x.code?'selected':''}}"><td><div class="company"><i class="radio"></i><div><strong>${{x.name}}</strong><small>${{x.summary.slice(0,18)}}…</small></div></div></td><td>${{x.code}}.HK</td><td>${{x.industry}}</td><td class="price">${{x.price}}<small>HKD</small></td><td>${{x.end}}</td><td>${{x.sub==null?'待补充':x.sub.toFixed(2)+'x'}}</td><td><strong>${{x.total.toFixed(1)}}</strong></td><td><span class="tier ${{tierClass(x.tier)}}">${{x.tier}}</span></td></tr>`).join('');$('foot').textContent=`共 ${{list.length}} 条 · 点击公司查看评分详情`;$('rows').querySelectorAll('tr').forEach(r=>r.onclick=()=>{{selected=DATA.find(x=>x.code===r.dataset.code);detail();render();$('panel').classList.remove('closed')}})}}function detail(){{if(!selected)return;const x=selected;$('pname').textContent=x.name;$('pcode').textContent=x.code+'.HK';$('ptier').className='tier '+tierClass(x.tier);$('ptier').textContent=x.tier;$('pmeta').textContent=`招股价 ${{x.price}} HKD　截止 ${{x.end}}　结果 ${{x.result}}　资金上限 ${{x.allocation?'HK$'+x.allocation.toLocaleString():'未纳入'}}`;$('dims').innerHTML=x.scores.map((v,i)=>`<div class="dimension"><label>${{labels[i]}} <small>${{weights[i]}}</small></label><div class="meter"><i style="width:${{Math.max(0,Math.min(100,v/weights[i]*100))}}%"></i></div><b>${{v}}</b></div>`).join('');$('base').textContent=x.total.toFixed(1);$('final').textContent=x.total.toFixed(1);$('quality').innerHTML=x.quality.length?x.quality.map(q=>`<p>${{q}}</p>`).join(''):'待补充';$('risks').innerHTML=x.risks.map(r=>`<li>${{r}}</li>`).join('');$('conclusion').textContent=x.summary}}const industries=[...new Set(DATA.map(x=>x.industry))].sort();$('industry').innerHTML+=[...industries].map(x=>`<option>${{x}}</option>`).join('');$('industry').onchange=render;$('tier').onchange=render;$('reset').onclick=()=>{{$('industry').value='';$('tier').value='';render()}};$('close').onclick=()=>$('panel').classList.add('closed');$('open').onclick=()=>$('panel').classList.remove('closed');document.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>alert('该单文件展示“今日分析”；完整历史与规则见 Markdown/PDF 报告。'));detail();render();if(window.innerWidth<=1100)$('panel').classList.add('closed');</script></body></html>"""
