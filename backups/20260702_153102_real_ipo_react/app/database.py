        ...
        if count == 0:
            ...
            for row in rows:
                ...
                if row["is_sample"] and row["code"] in sample_metrics:
                    scoring_keys = {"has_cornerstone", "cornerstone_quality_good", "greenshoe", "is_ah",
                                    "ah_premium", "peer_valuation_discount", "sponsor_quality_good"}
                    metrics.update({key: value for key, value in sample_metrics[row["code"]].items()
