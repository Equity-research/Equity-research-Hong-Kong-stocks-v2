...
import { api } from './api'
import type { IPODetail, IPO, Report } from './types'
...
function Header({ page, onPage }: { page: Page; onPage: (p: Page) => void }) {
  ...
  return <header className="topbar">
    ...
    <div className="source"><span className="status-dot" />数据源：样例 SQLite</div>
  </header>
}

function Summary({ items }: { items: IPO[] }) {
  const avg = items.length ? items.reduce((sum, item) => sum + item.final_score, 0) / items.length : 0
  const recommendations = [...items.filter(item => item.recommendation === '申购')]
  ...
  return <section className="summary" aria-label="今日汇总">{values.map(([label, value, color]) =>
    ...
    <div className="mobile-ranking">
      <span className="ranking-title">本次报告推荐打新排名</span>
      {recommendations.length ? <ol>{recommendations.map(item => <li key={item.id}>
        <span className="ranking-company"><b>{item.name}</b></span><RecommendationBadge value={item.recommendation}/>
      </li>)}</ol> : <p>暂无推荐打新的新股</p>}
    </div>
  </section>
}
...
function IPOTable({ items, selected, onSelect }: { items: IPO[]; selected?: number; onSelect:(id:number)=>void }) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>公司</th><th>代码</th><th>行业</th><th>招股价</th><th>截止日期</th><th>原始分</th><th>调整</th><th>最终分</th><th>建议</th></tr></thead>
    <tbody>{items.map(item => <tr key={item.id} className={selected === item.id ? 'selected' : ''} onClick={() => onSelect(item.id)}>
      <td><span className="radio"/><strong>{item.name}</strong><small>{item.english_name}</small></td><td>{item.code}</td><td>{item.industry}</td>
      <td>{item.price_low.toFixed(2)}–{item.price_high.toFixed(2)}<small>HKD</small></td><td>{item.deadline}</td><td>{item.original_score.toFixed(1)}</td>
...
function DetailPanel({ detail, onClose, onSaved }: { detail: IPODetail | null; onClose:()=>void; onSaved:()=>void }) {
  ...
  return <aside className="detail-panel" aria-label="IPO详情">
    ...
    <section><h3>评分维度 <small>（原始分 / 权重）</small></h3>{detail.dimensions.map(d => <div className="dimension" key={d.key} title={d.reasons.join('\n')}>
      <span>{d.label} <small>{d.weight}</small></span><div className="meter"><i style={{ width: `${d.score / d.weight * 100}%` }}/></div><strong>{d.score.toFixed(1)}</strong></div>)}</section>
    <div className="equation"><div><span>原始分</span><strong>{detail.original_score.toFixed(1)}</strong></div><b>+</b><div><span>调整</span><strong className={value > 0 ? 'green' : value < 0 ? 'red' : ''}>{value > 0 ? '+' : ''}{value}</strong></div><b>=</b><div><span>最终分</span><strong className="accent">{(detail.original_score + value).toFixed(1)}</strong></div></div>
    <section><h3>风险提示</h3><ul className="risks">{detail.risks.map(r => <li key={r}>{r}</li>)}</ul></section>
    <section className="adjust"><h3>手动调整 <small>（−10 至 +10）</small></h3><div className="range-row"><span>−10</span><input aria-label="手动调整" type="range" min="-10" max="10" step="1" value={value} onChange={e => setValue(Number(e.target.value))}/><span>+10</span><output>{value > 0 ? '+' : ''}{value}</output></div>
...
function TodayPage({ onReports }: { onReports:()=>void }) {
  ...
  return <main className="dashboard"><div className="workspace"><div className="content">
    <div className="page-title"><div><h1>今日 IPO 分析</h1><p>样例数据 · 透明评分 · 仅供研究演示</p></div><button className="primary" onClick={createReport} disabled={generating}><FileText size={18}/>{generating ? '生成中…' : '生成今日日报'}</button></div>
    {error && <div className="alert">{error}<button onClick={() => load()}>重试</button></div>}
    {loading ? <div className="loading">正在加载样例数据…</div> : <><Summary items={items}/><FilterBar industry={industry} recommendation={recommendation} onIndustry={setIndustry} onRecommendation={setRecommendation} onReset={() => {setIndustry('');setRecommendation('')}} industries={[...new Set(items.map(i=>i.industry))]}/><IPOTable items={visible} selected={selected} onSelect={openDetail}/><p className="count">共 {visible.length} 条 · 所有公司与数值均为样例</p></>}
...
function IPOPage() {
  ...
  return <main className="simple-page"><div className="section-title"><SlidersHorizontal/><div><h1>IPO 项目</h1><p>查看全部样例项目及其评分结果</p></div></div><IPOTable items={items} selected={detail?.id} onSelect={id => api.getIPO(id).then(setDetail)}/><DetailPanel detail={detail} onClose={()=>setDetail(null)} onSaved={()=> detail && api.getIPO(detail.id).then(setDetail)}/></main>
}
...
function RulesPage() {
  const dimensions = [['基本面',30,'营收增长、利润增长、净利率、负债率'],['发行结构',25,'基石配售、认购倍数、公众持股、摊薄比例'],['行业',15,'行业景气度'],['估值',20,'同业 PE/PS 折价、定价吸引力'],['市场情绪',10,'新闻情绪、认购动能']]
  return <main className="simple-page"><div className="section-title"><BookOpen/><div><h1>评分规则</h1><p>所有权重与阈值均来自 scoring_rules.yaml</p></div></div><div className="rules-grid">{dimensions.map(([name,weight,desc]) => <article key={String(name)}><strong>{name}</strong><b>{weight} 分</b><p>{desc}</p></article>)}</div><section className="method"><h2>计算与推荐</h2><p>每个指标按配置的最小值、最大值进行 0–1 线性归一化，再乘以指标权重。缺失指标按 50% 中性值计分并明确提示。</p><div className="thresholds"><span className="buy">≥75　申购</span><span className="hold">55–74.99　观望</span><span className="avoid">&lt;55　回避</span></div><p>人工调整限制为 −10 至 +10，必须填写至少 10 个字符的原因；原始规则分始终保留。</p></section></main>
}

export default function App() {
  const [page, setPage] = useState<Page>('today')
  return <><Header page={page} onPage={setPage}/>{page === 'today' && <TodayPage onReports={()=>setPage('reports')}/>} {page === 'ipos' && <IPOPage/>}{page === 'reports' && <ReportsPage/>}{page === 'rules' && <RulesPage/>}<footer>免责声明：本系统数据均为样例，仅供研究参考，不构成任何投资建议。投资有风险，入市需谨慎。</footer></>
}
